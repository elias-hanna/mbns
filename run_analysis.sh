...
dump_vals=(1000 2000 3000 4000 5000 6000 7000 8000 9000 10000)
...
for env in "${environments[@]}"; do
    ...
    python ~/Documents/thesis/dev/model_init_exps/daqd/vis_repertoire_mis_transfer_sel.py --init-methods ${methods[*]} --init-episodes ${episodes[*]} --fitness-funcs ${fitnesses[*]} --transfer-selection ${transfer_sels[*]} --nb-transfer ${nb_transfers[*]} --environment $env --dump-vals ${dump_vals[*]} --dump-path .
    cd ..
    cpt=$((cpt+1))
    echo "finished archive analysis for $env"
done
