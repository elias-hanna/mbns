...
from src.torch import pytorch_util as ptu
from src.torch.modules import LayerNorm
...
class ParallelizedEnsemble(nn.Module):
    ...
    def __init__(
        ...
        self.input_mu = nn.Parameter(
            ptu.zeros(1, input_size), requires_grad=False).float()
        self.input_std = nn.Parameter(
            ptu.ones(1, input_size), requires_grad=False).float()

        self.output_mu = nn.Parameter(
            ptu.zeros(1,int(output_size/2)), requires_grad=False).float()
        self.output_std = nn.Parameter(
            ptu.ones(1,int(output_size/2)), requires_grad=False).float()
