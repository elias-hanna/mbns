...
class DeterministicQDSurrogate(nn.Module):
    """
    Predicts bd and fitness given genotype
    """
    def __init__(
            self,
            gen_dim,              # Observation dim of environment
            bd_dim,           # Action dim of environment
            hidden_size,         # Hidden size for model
    ):
        super(DeterministicQDSurrogate, self).__init__()

        torch.set_num_threads(16)
        
        self.gen_dim, self.bd_dim = gen_dim, bd_dim
