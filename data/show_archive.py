...
def plot_archive(data, plt, args, ss_min, ss_max, bounds=False, name="", bd_col=0, c_by='bd',
    ...
    if args.plot_type == "grid":
        ...
        if not real_and_model:
            ...
            data.plot.scatter(x='bd0',y='bd1',c='fit',colormap='viridis', s=10, ax=ax)
            ...
            if c_by == 'bd':
                data.plot.scatter(x='bd0_model',y='bd1_model',c=f'bd{bd_col}_model',
                                  colormap='viridis',s=10, ax=ax)
            elif c_by == 'fit':
                print(data)
                data.plot.scatter(x='bd0_model',y='bd1_model',c='fit_model',
                                  colormap='viridis',s=10, ax=ax)
            elif c_by == 'idx':
                data.plot.scatter(x='bd0_model',y='bd1_model',c=colors,s=10, ax=ax)
            if name:
                fig, ax = plt.subplots()
                if c_by == 'bd':
                    data.plot.scatter(x=f'bd0_{name}',y=f'bd1_{name}',c=f'bd{bd_col}_model',
                                      colormap='viridis',s=10, ax=ax)
                elif c_by == 'fit':
                    data.plot.scatter(x=f'bd0_{name}',y=f'bd1_{name}',c=f'fit_{name}',
                                      colormap='viridis',s=10, ax=ax)
                elif c_by == 'idx':
                    data.plot.scatter(x=f'bd0_{name}',y=f'bd1_{name}',c=colors,s=10, ax=ax)
...
def get_closest_to_cluster_centroid(data, n_clusters=100):
    ...
    for i in range(n_clusters):
        ...
        cluster['distance'] = ((cluster['bd0'] - cluster_center[0])**2 + (cluster['bd1'] - cluster_center[1])**2)**0.5
...
def get_novelty_scores(data, k=15):
    from sklearn.neighbors import NearestNeighbors
    
    # Convert the dataset to a numpy array
    dataset = np.array(data[['bd0', 'bd1']])
...
def get_most_nov_data(data, n=100):
    data_nov = get_novelty_scores(data)
...
def main(args):
    ...
    if args.model_and_real:
        ...
        data_most_nov = get_most_nov_data(data, n=100)
    
        data_centers = get_closest_to_cluster_centroid(data, n_clusters=100)
        ...
        fig, ax = plot_archive(df_merged_all, plt, args, ss_min, ss_max,
                               ...
                               c_by='fit', bd_col=1, real_and_model=True)
